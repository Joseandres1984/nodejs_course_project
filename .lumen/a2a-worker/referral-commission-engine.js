...
export async function planReferralCommissions(env) {
  ...
  let rows = [];
  try {
    const result = await env.DB.prepare("SELECT id,estimated_value_usd,commission_status,status FROM lumen_referrals WHERE commission_status='NOT_CONFIGURED' AND status NOT IN ('BLOCKED_TRUST','SETTLED') ORDER BY updated_at DESC LIMIT 120").all();
    rows = result.results || [];
  } catch {
  ...
  for (const ref of rows) {
    ...
    const proposedAmount = dealValue > 0 ? Math.max(1, Math.round((dealValue * DEFAULT_PROPOSED_RATE_PCT / 100) * 100) / 100) : null;
    const status = dealValue > 0 ? "PROPOSAL_READY" : "BASIS_REQUIRED";
    const id = commissionId(ref.id);
    await env.DB.prepare("INSERT INTO lumen_referral_commissions(id,referral_id,created_at,updated_at,basis_type,deal_value_usd,proposed_rate_pct,proposed_amount_usd,status,engine_version) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(referral_id) DO UPDATE SET updated_at=excluded.updated_at,deal_value_usd=excluded.deal_value_usd,proposed_rate_pct=excluded.proposed_rate_pct,proposed_amount_usd=excluded.proposed_amount_usd,engine_version=excluded.engine_version")
      .bind(id, ref.id, now, now, "SUCCESS_FEE_PERCENT", dealValue || null, DEFAULT_PROPOSED_RATE_PCT, proposedAmount, status, VERSION).run();
    await env.DB.prepare("UPDATE lumen_referrals SET commission_status=?,updated_at=? WHERE id=? AND commission_status='NOT_CONFIGURED'")
      .bind(status, now, ref.id).run();
    await logReferralEvent(env, ref.id, "COMMISSION_CASE_PLANNED", `status=${status};internal_proposed_rate_pct=${DEFAULT_PROPOSED_RATE_PCT};non_binding=true`, proposedAmount, false);
    planned.push({ referralId:ref.id, commissionId:id, status, dealValueUsd:dealValue || null, proposedRatePct:DEFAULT_PROPOSED_RATE_PCT, proposedAmountUsd:proposedAmount });
  }
  return { ok:true, version:VERSION, planned:planned.length, cases:planned.slice(0,50), guardrails:{ proposalIsInternalOnly:true, counterpartyNotContacted:true, agreementRequiredBeforePayment:true, autonomousSpend:false, bindingActionsHumanGated:true } };
}
...
export async function syncReferralCommissionSettlements(env) {
  if (!(await ensureSchema(env))) return { ok:false, error:"persistence_unavailable", version:VERSION };
  const result = await env.DB.prepare("SELECT * FROM lumen_referral_commissions WHERE status IN ('PAYMENT_DUE','AGREED_PENDING_CLOSE') ORDER BY updated_at ASC LIMIT 150").all();
  const settled = [];
  for (const row of result.results || []) {
    let event = null;
    try {
      event = await env.DB.prepare("SELECT id,amount_usd,evidence,metadata,created_at FROM lumen_revenue_events WHERE event_type='payment_settled' AND status='verified' AND item_id=? ORDER BY created_at ASC LIMIT 1")
        .bind(row.referral_id).first();
    } catch {}
    if (!event) continue;
    ...
    await env.DB.prepare("UPDATE lumen_referrals SET commission_status='SETTLED',settled_revenue_usd=?,settlement_event_id=?,updated_at=? WHERE id=?")
  ...
  return { ok:true, version:VERSION, settled:settled.length, results:settled, guardrails:{ verifiedPaymentEventRequired:true, underpaymentsNotRecognizedAsFullSettlement:true, autonomousSpend:false } };
}

async function statsData(env) {
  await ensureSchema(env);
  const row = await env.DB.prepare("SELECT COUNT(*) total,SUM(CASE WHEN status='BASIS_REQUIRED' THEN 1 ELSE 0 END) basis_required,SUM(CASE WHEN status='PROPOSAL_READY' THEN 1 ELSE 0 END) proposal_ready,SUM(CASE WHEN status='AGREED_PENDING_CLOSE' THEN 1 ELSE 0 END) agreed,SUM(CASE WHEN status='PAYMENT_DUE' THEN 1 ELSE 0 END) payment_due,SUM(CASE WHEN status='SETTLED' THEN 1 ELSE 0 END) settled,COALESCE(SUM(CASE WHEN status='PAYMENT_DUE' THEN agreed_amount_usd ELSE 0 END),0) outstanding,COALESCE(SUM(CASE WHEN status='SETTLED' THEN settled_amount_usd ELSE 0 END),0) realized FROM lumen_referral_commissions").first();
  return { total:Number(row?.total || 0), basisRequired:Number(row?.basis_required || 0), proposalReady:Number(row?.proposal_ready || 0), agreedPendingClose:Number(row?.agreed || 0), paymentDue:Number(row?.payment_due || 0), settled:Number(row?.settled || 0), outstandingCommissionUsd:Number(row?.outstanding || 0), realizedCommissionRevenueUsd:Number(row?.realized || 0) };
}
