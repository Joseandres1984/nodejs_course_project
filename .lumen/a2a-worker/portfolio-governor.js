...
function authorized(request, env) { const configured = clean(env?.OPPORTUNITY_ADMIN_TOKEN, 500); const provided = clean(request.headers.get("x-lumen-admin"), 500); return Boolean(configured && provided && configured === provided); }
...
async function safeAll(env, sql, binds = []) { try { const q = env.DB.prepare(sql); const r = binds.length ? await q.bind(...binds).all() : await q.all(); return r.results || []; } catch { return []; } }
async function safeNumber(env, sql, binds = []) { try { const q = env.DB.prepare(sql); const r = binds.length ? await q.bind(...binds).first() : await q.first(); return num(r?.n); } catch { return 0; } }
...
function normalizeCandidate(row) {
  if (!row) return null;
  return { id: row.id, sourceType: row.source_type, sourceId: row.source_id, businessFamily: row.business_family || businessFamily(row), lane: row.lane, stage: row.stage, title: row.title || null, estimatedValueUsd: num(row.estimated_value_usd), probability: num(row.probability), urgency: num(row.urgency), evidenceScore: num(row.evidence_score), signalScore: num(row.signal_score), economicScore: num(row.economic_score), autonomyScore: num(row.autonomy_score), feedbackAdjustment: num(row.feedback_adjustment), explorationBonus: num(row.exploration_bonus), actionKind: row.action_kind, actionRef: row.action_ref || null, rationale: row.rationale || null, updatedAt: row.updated_at };
}
...
function activeAttention(counts) {
  ...
  let allocated = 0;
  active.forEach((lane, index) => { if (index === active.length - 1) result[lane] = 100 - allocated; else { const share = Math.round(BASE_ATTENTION[lane] * 100 / denominator); result[lane] = share; allocated += share; } });
  return result;
}
...
function familyAttention(rows) {
  ...
  const weights = {};
  for (const family of BUSINESS_FAMILIES) { const members = groups[family] || []; const top = members.reduce((m, row) => Math.max(m, num(row.autonomy_score)), 0); weights[family] = members.length ? Math.max(5, top) ** 1.35 : 0; }
  const total = Object.values(weights).reduce((a, b) => a + b, 0);
  if (!total) return Object.fromEntries(BUSINESS_FAMILIES.map(f => [f, 0]));
  const result = {}; let allocated = 0; const active = BUSINESS_FAMILIES.filter(f => weights[f] > 0);
  active.forEach((family, index) => { if (index === active.length - 1) result[family] = Math.max(0, 100 - allocated); else { const share = Math.max(1, Math.round(weights[family] * 100 / total)); result[family] = share; allocated += share; } });
...
function explorationBonusFor(family, learning) { const row = learning[family] || {}; const selected = Math.max(0, num(row.selected_cycles)); if (selected === 0) return 6; if (selected <= 2) return 4; if (selected <= 5) return 2; return 0; }
...
async function virtualCommerceCandidates(env) {
  ...
  for (const row of paid) rows.push({ id: `VCOM-ORDER-${clean(row.event_id, 140)}`, source_type: "COMMERCE_ORDER", source_id: row.event_id, lane: "CLOSE", stage: row.event_type, title: `Verified commerce order ${row.external_order_id}`, estimated_value_usd: 0, probability: 0.9, urgency: 0.95, evidence_score: 94, signal_score: 90, economic_score: 0, action_kind: "COMMERCE_FULFILLMENT_REVIEW", action_ref: row.external_order_id, rationale: "verified_paid_commerce_order_requires_governed_fulfillment;currency_not_assumed_usd", updated_at: row.received_at });
  ...
  for (const row of drafts) {
    const published = num(row.published) > 0 || clean(row.status, 30).toUpperCase() === "PUBLISHED"; const samples = Math.max(0, num(row.market_sample_count)); const margin = clamp(row.projected_margin_pct, 0, 60);
    rows.push({ id: `VCOM-DRAFT-${clean(row.draft_id, 140)}`, source_type: "COMMERCE_PRODUCT", source_id: row.draft_id, lane: published ? "NEW_BUSINESS" : "EXPERIMENT", stage: published ? "PUBLISHED" : row.status, title: row.title || row.draft_id, estimated_value_usd: 0, probability: published ? 0.12 : 0.06, urgency: published ? 0.32 : 0.12, evidence_score: Math.min(82, 48 + Math.min(20, samples * 2) + (clean(row.competitiveness, 50) === "COMPETITIVE" ? 10 : 0)), signal_score: Math.min(90, 35 + margin), economic_score: 0, action_kind: published ? "COMMERCE_OBSERVE_CONVERSION" : "COMMERCE_REVIEW", action_ref: row.draft_id, rationale: published ? "commerce_is_live_but_must_earn_attention_from_real_conversion" : "commerce_candidate_remains_an_experiment_until_market_and_conversion_evidence_improve", updated_at: row.updated_at });
  }
  return rows;
}

function enrichRows(rows, feedbackMap, learning) { return rows.map(row => { const family = businessFamily(row); const feedback = num(feedbackMap.get(`${clean(row.source_type, 80)}:${clean(row.source_id, 180)}`)); const exploration = explorationBonusFor(family, learning); return { ...row, business_family: family, feedback_adjustment: feedback, exploration_bonus: exploration, autonomy_score: economicPriority(row, feedback, exploration) }; }); }

export function chooseEconomicCandidate(rows, { externalOnly = false } = {}) {
  let candidates = [...(rows || [])].filter(Boolean); if (externalOnly) candidates = candidates.filter(row => EXTERNAL_ACTIONS.has(row.action_kind)); if (!candidates.length) return null;
  const collection = candidates.filter(row => clean(row.lane, 30).toUpperCase() === "COLLECTION"); const pool = collection.length ? collection : candidates;
...
export async function recomputePortfolioGovernor(env) {
  ...
  await env.DB.prepare("INSERT INTO lumen_portfolio_governor_runs(id,created_at,recommended_lane,recommended_action,active_candidates,verified_revenue_usd,details_json,engine_version) VALUES(?,?,?,?,?,?,?,?)").bind(runId, now, recommendedLane, recommendedAction, rows.length, verifiedRevenueUsd, JSON.stringify({ counts, attention, familyCounts, familyAttentionPct, recommendedFamily, top: normalizedTop, external: normalizedExternal, selection: "economic_truth_first" }).slice(0, 8000), VERSION).run();
  return { ok: true, version: VERSION, compatibilityVersion: COMPATIBILITY_VERSION, cycle, objective: "maximize_verified_revenue_probability_under_zero_spend_and_authority_constraints", recommendedBusinessFamily: recommendedFamily, recommendedLane, recommendedExternalAction: recommendedAction, attention, businessFamilyAttention: familyAttentionPct, metrics, topCandidate: normalizedTop, externalCandidate: normalizedExternal, guardrails: { selectionRule: "collection_if_due_else_highest_economic_autonomy_score_across_business_families", verifiedRevenueFeedbackApplied: true, explorationBonusMax: 6, commerceIsCapabilityNotDefaultStrategy: true, existingBusinessProtected: true, collectionBeforeExploration: true, technicalRepliesDoNotCreateRevenueCredit: true, prioritizationDoesNotChangePrices: true, maxExternalMessagesPerCycle: 1, autonomousSpend: false, autonomousSpendUsd: 0, autonomousPurchase: false, autonomousContract: false, bindingActionsHumanGated: true } };
}

async function state(env) {
  if (!(await ensureSchema(env))) return { version: VERSION, initialized: false };
  const row = await env.DB.prepare("SELECT * FROM lumen_portfolio_governor_state WHERE id='GLOBAL' LIMIT 1").first(); if (!row) return { version: VERSION, compatibilityVersion: COMPATIBILITY_VERSION, initialized: false, objective: "maximize_verified_revenue_probability_under_zero_spend_and_authority_constraints", baseAttention: BASE_ATTENTION };
  const parse = (v, fallback) => { try { return JSON.parse(v || ""); } catch { return fallback; } }; const metrics = parse(row.metrics_json, {});
  return { version: VERSION, compatibilityVersion: COMPATIBILITY_VERSION, initialized: true, updatedAt: row.updated_at, cycle: num(row.cycle), recommendedBusinessFamily: metrics.recommendedBusinessFamily || null, recommendedLane: row.recommended_lane, recommendedExternalAction: row.recommended_action, recommendedSourceType: row.recommended_source_type, recommendedSourceId: row.recommended_source_id, attention: parse(row.attention_json, {}), businessFamilyAttention: metrics.businessFamilyAttention || {}, metrics, topCandidate: parse(row.top_candidate_json, null), externalCandidate: parse(row.external_candidate_json, null), guardrails: { maxExternalMessagesPerCycle: 1, autonomousSpend: false, autonomousSpendUsd: 0, autonomousPurchase: false, autonomousContract: false, bindingActionsHumanGated: true } };
}

export async function handlePortfolioGovernor(request, env) {
  const url = new URL(request.url);
  if (request.method === "GET" && url.pathname === "/portfolio-governor/policy") return json({ version: VERSION, compatibilityVersion: COMPATIBILITY_VERSION, name: "LUMEN Economic Autonomy Governor", objective: "maximize_verified_revenue_probability_under_zero_spend_and_authority_constraints", businessFamilies: BUSINESS_FAMILIES, selectionRule: "collection_if_due_else_probability_x_evidence_x_urgency_x_time_to_cash_x_verified_profit_feedback_plus_bounded_exploration", externalActionRule: "best_safe_external_action_by_economic_autonomy_score_with_at_most_one_external_message_per_cycle", learningRule: "verified_settlement_feedback_adjusts_service_and_a2a_priority; repeated_no_settlement_can_reduce_priority; cold_start_is_neutral", commerceRule: "commerce_competes_as_one_capability_and_receives_no_special_priority_without_conversion_evidence", changesPrices: false, createsContracts: false, autonomousSpend: false, autonomousSpendUsd: 0, autonomousPurchase: false, maxExternalMessagesPerCycle: 1, bindingActionsHumanGated: true });
  if (request.method === "GET" && url.pathname === "/portfolio-governor/state") return json(await state(env));
  if (request.method === "POST" && url.pathname === "/portfolio-governor/recompute") { if (!authorized(request, env)) return json({ ok: false, error: "admin_token_required" }, 403); return json(await recomputePortfolioGovernor(env), 202); }
  return null;
}
