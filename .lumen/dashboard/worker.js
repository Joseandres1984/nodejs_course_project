...
function summarize(state, manifest) {
  ...
  return {
    status:{
      ...
      fleet_size:fleetSize, worker_status:s(workforce.status,fleetSize?"healthy":"unknown"), persistence:"Cloudflare D1", state_bytes:n(manifest.uncompressed_bytes),
      cycle:n(workforce.company_cycle || state.ticks)
    },
    ...
    intelligence:{
      market_status:s(marketIntel.status,"active"), authority:s(marketIntel.authority,"research_only"),
      observations:n(marketIntel.observations_total), products:n(expansion.products_known), companies:n(expansion.companies_known), catalogs:n(expansion.catalogs_indexed),
      lane:s(continuous.primary_lane,"—"), action:s(continuous.primary_action,"—"), learning_status:s(learning.status,"active"), revenue_os:s(revenueOS.status,"active")
    },
    governance:{
...
<section class="page" id="inquiries"><div class="grid g3"><div class="card"><div class="klabel">Consultas web</div><div class="kpi" id="inquiryCount">–</div></div><div class="card"><div class="klabel">Servicios activos</div><div class="kpi">6</div></div><div class="card"><div class="klabel">Precio inicial</div><div class="kpi">USD 59</div></div></div><div class="card section"><h2>Consultas públicas recientes</h2><div id="inquiriesTable"></div></div></section>
<section class="page" id="infra"><div class="grid g2"><div class="card"><h2>Infraestructura</h2><div id="infraStatus"></div></div><div class="card"><h2>Gobernanza</h2><div id="governance"></div></div></div><div class="card section"><h2>Diario de ciclos</h2><div id="journal"></div></div></section>
<section class="page" id="activity"><div class="grid g2"><div class="card"><h2>Pendientes priorizados</h2><div id="pending"></div></div><div class="card"><h2>Actividad</h2><div class="activity" id="activityList"></div></div></div></section><footer class="footer">LUMEN Zero · un solo panel privado para operación, control y trazabilidad. El sitio comercial continúa público por diseño.</footer></div><script>
...
async function loadData(){try{const r=await fetch('/api/data',{cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();render(d)}catch(e){$('result').innerHTML='<div class="result" style="border-color:#7a3d3d;background:#35191c;color:#ffc4be">No pude leer el estado: '+esc(e.message)+'</div>'}}
function render(d){
...
$('priority').innerHTML=line('Carril',esc(d.intelligence.lane))+line('Acción',esc(d.intelligence.action))+line('Bloqueo externo',esc(d.outbound.blocker||'ninguno'))+line('Elegibles',d.outbound.eligible)+line('Búsquedas restantes',d.scout.remaining)+'<p class="note">LUMEN prioriza progreso comercial verificable; no convierte señales débiles en oportunidades reales.</p>';
$('channels').innerHTML=line('Gmail',yn(d.outbound.mail_ready))+line('Instagram',yn(d.outbound.instagram))+line('Web pública',d.services.public_web.status==='online'?'<span class="chip ok">ONLINE</span>':'<span class="chip no">'+esc(d.services.public_web.status)+'</span>')+line('WhatsApp','<span class="chip">POSTERGADO</span>')+line('Pagos','<span class="chip">HUMANO</span>');
  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      try {
        ...
        return Response.json({ok:true,service:"lumen-unified-command-center",persistence:"cloudflare_d1",updated_at:manifest.updated_at},{headers:{"Cache-Control":"no-store"}});
      } catch (e) {
