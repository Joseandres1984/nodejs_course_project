...
function rootState(payload) {
  if (payload && payload.module_data && typeof payload.module_data === "object") return payload.module_data;
...
function summarizeExperiment(state) {
  ...
  return {
    ...
    sample_clicks: num(current.observed_clicks ?? current.baseline_clicks), sample_leads: num(current.observed_leads ?? current.baseline_leads),
    sample_verified_companies: num(current.observed_verified_companies ?? current.baseline_verified_companies),
    conversion_rate: num(current.observed_conversion_rate ?? current.baseline_conversion_rate),
    product_winner: str(engine.winner_product || current.product_winner || product.product_slug, "sin muestra suficiente"),
    product_evidence: str(product.evidence_class, "—"),
    channel_winner: str(engine.winner_channel || current.channel_winner || channel.source, "sin muestra suficiente"),
    channel_campaign: str(current.channel_winner_campaign || channel.campaign, "—"),
    outcome_samples: samples,
    verified_settlements: num(engine.verified_settlements || current.verified_settlements || product.settled_count || derivedSettlements),
    verified_revenue_usd: num(engine.verified_revenue_usd || current.verified_revenue_usd || product.realized_revenue_usd),
    bottleneck: str(engine.bottleneck || current.bottleneck || director.bottleneck, "—"),
    ...
    history,
    guardrails: {
      monetary_budget_usd: num(obj(engine.guardrails).monetary_budget_usd),
      paid_media_authority_changed: obj(engine.guardrails).paid_media_authority_changed === true,
      binding_authority_changed: obj(engine.guardrails).binding_authority_changed === true,
      search_budget_increased: obj(engine.guardrails).search_budget_increased === true,
      hard_bottleneck_override: obj(engine.guardrails).hard_bottleneck_override === true,
    },
  };
}
...
function inject(html) {
  ...
  if (commercialTab.test(html)) {
    html = html.replace(commercialTab, (match) => match + '<button class="tab" data-p="experiments">Experimentos</button>');
  } else {
    const tabsOpen = /(<div\b[^>]*class=["'][^"']*\btabs\b[^"']*["'][^>]*>)/i;
    html = html.replace(tabsOpen, (match) => match + '<button class="tab" data-p="experiments">Experimentos</button>');
  }

  if (html.includes('<section class="page" id="infra">')) {
    html = html.replace('<section class="page" id="infra">', SECTION + '<section class="page" id="infra">');
  } else {
...
export { inject as injectExperimentHtml, rootState, summarizeExperiment };
