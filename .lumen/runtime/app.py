...
app = FastAPI(title="LUMEN B2B", version="1.2-online-es")
security = HTTPBasic()
...
STATE: Dict[str, object] = {
    ...
    "standing_goals": [
        ...
        "Proteger margen, reputación y capital",
    ],
    "ticks": 0,
}
...
def seed_demo():
    ...
    STATE["buyers"] = [
        ...
        {"name":"Río Sur Ingeniería","sector":"Servicios de energía","need":"fuentes de alimentación e instrumentación","fit":91,"budget":81},
    ...
    STATE["suppliers"] = [
        ...
        {"name":"TecnoSource LATAM","category":"fuentes de alimentación e instrumentación","reliability":89,"price":90,"lead":81},
        {"name":"Industrial Hub","category":"consumibles técnicos","reliability":82,"price":88,"lead":90},
    ]
    log("Supplier Hunter y Buyer Hunter construyeron una red B2B de demostración.")
...
def build_opportunities():
    ...
    for b in STATE["buyers"]:
        ...
        opp = {
            "id":f"OPP-{len(STATE['opportunities'])+1:04d}",
            "buyer":b["name"], "supplier":s["name"], "need":b["need"],
            "score":score, "pipeline":value, "margin":margin_value,
            "status":"calificada" if score >= 70 else "investigación",
        }
        STATE["opportunities"].append(opp)
        created += 1
        log(f"Opportunity Radar creó {opp['id']} — {b['name']} ↔ {s['name']} (puntaje {score}).")
    return created
...
def open_deals():
    ...
    for o in STATE["opportunities"]:
        ...
        d = {
            "id":f"DEAL-{len(STATE['deals'])+1:04d}", "opportunity_id":o["id"],
            "buyer":o["buyer"], "supplier":o["supplier"], "stage":"descubrimiento",
            "close_prob":close_prob, "expected_value":ev, "margin":o["margin"],
            "next_action":"Investigar al comprador y preparar un contacto personalizado",
        }
...
def autopilot_tick():
    ...
    if c1 == 0 and c2 == 0:
        if STATE["deals"]:
            ...
            stages = ["descubrimiento", "calificada", "propuesta", "negociación"]
            ...
            if idx < len(stages)-1:
                ...
                log(f"LUMEN DRIVE avanzó {d['id']} a {d['stage']} después de repriorizar por valor esperado.")
            else:
...
@app.get("/health")
def health():
    return {"ok": True, "service":"lumen-b2b", "version":"1.2-online-es", "time": now()}
...
def api_signal(company: str = Form(...), need: str = Form(...), fit: int = Form(80), _=Depends(auth)):
    STATE["buyers"].append({"name":company,"sector":"señal del usuario","need":need,"fit":max(1,min(100,fit)),"budget":75})
...
def dashboard(_=Depends(auth)):
    ...
    opp_rows = "".join(
        f"<tr><td>{o['id']}</td><td>{o['buyer']}</td><td>{o['need']}</td><td class='score'>{o['score']}</td><td>{money(o['pipeline'])}</td></tr>"
        for o in sorted(STATE["opportunities"], key=lambda x:x["score"], reverse=True)[:8]
    ) or "<tr><td colspan=5>Todavía no hay oportunidades</td></tr>"
    deal_rows = "".join(
        f"<tr><td>{d['id']}</td><td>{d['buyer']}</td><td><span class='status'>{d['stage']}</span></td><td>{int(d['close_prob']*100)}%</td><td>{money(d['expected_value'])}</td></tr>"
        for d in sorted(STATE["deals"], key=lambda x:x["expected_value"], reverse=True)[:8]
    ) or "<tr><td colspan=5>Todavía no hay negocios</td></tr>"
    events = "".join(
        f"<div class='event'><div>{e['msg']}</div><div class='time'>{e['ts']}</div></div>"
        for e in STATE["activity"][:16]
    ) or "<div class='event'>Autopilot listo.</div>"
    ...
    <div class='top'><div><div class='logo'>LUMEN</div><div class='sub'>Sistema Operativo de Emprendimiento B2B Autónomo · Centro de Comando</div></div><div class='badge'>● AUTOPILOT · {mode}</div></div>
    <div class='goals'>{goals}</div>
    <div class='grid section'><div class='card'><div class='label'>Pipeline comercial</div><div class='kpi'>{money(pipeline)}</div></div><div class='card'><div class='label'>Valor esperado ajustado por riesgo</div><div class='kpi'>{money(ev)}</div></div><div class='card'><div class='label'>Margen potencial</div><div class='kpi'>{money(margin)}</div></div><div class='card'><div class='label'>Ciclos de Autopilot</div><div class='kpi'>{STATE['ticks']}</div></div></div>
    <div class='two'><div><div class='card'><div class='label'>Radar de oportunidades</div><table><tr><th>ID</th><th>Comprador</th><th>Necesidad</th><th>Puntaje</th><th>Pipeline</th></tr>{opp_rows}</table></div><div class='card section'><div class='label'>Mesa de negocios</div><table><tr><th>ID</th><th>Comprador</th><th>Etapa</th><th>Prob. cierre</th><th>Valor esperado</th></tr>{deal_rows}</table></div></div>
    <div><div class='card'><div class='label'>Actividad de LUMEN</div><div class='activity'>{events}</div></div><div class='card section'><div class='label'>Darle una señal de mercado a LUMEN</div><form method='post' action='/api/signal'><input name='company' placeholder='Empresa' required><input name='need' placeholder='¿Qué podría necesitar?' required><input name='fit' type='number' min='1' max='100' value='85'><button>Dar señal a LUMEN</button></form><form class='section' method='post' action='/api/demo/reset'><button class='muted'>Reiniciar demostración</button></form></div></div></div>
    <div class='sub section'>Governor activo · Sin compromisos financieros autónomos · Sin contacto externo real salvo activación explícita.</div></div></body></html>"""
