...
def whatsapp_probe_tick(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    body = (
        "LUMEN · PRUEBA OK\n"
        "WhatsApp quedó conectado correctamente. Desde ahora LUMEN podrá avisarte eventos importantes cuando activemos el modo operativo.\n"
    ...
    try:
        ...
        attempt.update({"error": f"HTTPError:{exc.code}", "http_status": int(exc.code)})
        report.update({"status": "failed", "error": f"HTTPError:{exc.code}", "http_status": int(exc.code)})
