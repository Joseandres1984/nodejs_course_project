...
def _realized_profit(state: Dict[str, Any]) -> float:
    ...
    candidates = (
        finance.get("realized_profit_ars"),
        finance.get("realized_company_profit_ars"),
        finance.get("realized_profit_usd"),
        finance.get("realized_company_profit_usd"),
        snapshot.get("realized_profit_ars"),
        snapshot.get("realized_profit_usd"),
    )
    return max([_f(x) for x in candidates if x not in (None, "")] or [0.0])


def _cash_score(case: Dict[str, Any]) -> float:
    # Prefer verified cases already closer to quote/offer/close, while keeping a path for strong early demand.
    ...
    if case.get("human_required"):
        score -= 8.0  # do not crowd the queue with cases that cannot advance autonomously yet
...
def first_cash_tick(state: Dict[str, Any], autonomy: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    active = realized <= 0.0
    cases: List[Dict[str, Any]] = list(autonomy.get("cases", []) or [])
    ...
    if active:
        ...
        for case in top:
            ...
            queue.insert(0, {
                "key": key,
                "kind": "first_cash_priority",
                "title": f"FIRST CASH: {case.get('title') or case.get('category') or case.get('id')}",
                "reason": f"Priorizar la primera ganancia realizada. Etapa {case.get('stage')}; cash score {case.get('first_cash_score')}; siguiente acción: {case.get('next_action')}",
                "impact": 100.0,
                "urgency": 96.0,
                ...
                "payload": {
                    ...
                    "cash_score": case.get("first_cash_score"),
                    "source_type": case.get("source_type"),
                    "source_id": case.get("source_id"),
                },
                "priority_score": 100.0 - injected,
                "created_at": utcnow(),
            })
            existing.add(key)
            injected += 1
    ...
    report = {
        ...
        "realized_profit_detected": realized,
        "top_cash_cases": top,
        "actions_injected": injected,
        "policy": {
            "contracts_payments_orders_remain_human_gated": True,
            "nonbinding_research_rfq_followup_negotiation_can_be_autonomous": True,
