...
VERSION = "1.4-revenue-allocator-guarded-anti-drift"
_ORIGINAL_RUN = elastic_agent_fleet.run_elastic_agent_fleet_cycle

LANE_WEIGHTS = {
    "closing": {
        "buyer_hunter": 0.10,
        "supplier_hunter": 0.10,
        "market_scout": 0.04,
        "research_analyst": 0.14,
        "revops": 0.25,
        "negotiator": 0.16,
        "market_manager": 0.04,
        "risk_quality": 0.10,
    ...
    "opportunity_building": {
        "buyer_hunter": 0.20,
        "supplier_hunter": 0.17,
        "market_scout": 0.10,
        "research_analyst": 0.19,
        "revops": 0.16,
        "negotiator": 0.06,
        "market_manager": 0.04,
        "risk_quality": 0.05,
...
def _resolve_lane_with_anti_drift(
    state: Dict[str, Any],
    truth: Dict[str, Any],
    ...
    if triggered and suggestion in LANE_WEIGHTS and suggestion != base_lane:
        # A real, canonical close path is protected. Anti-drift may rotate discovery/research
        # attention, but it must not pull the system away from an evidence-backed closing path.
        if base_lane == "closing" and metrics.get("closing_eligible_deals", 0) > 0:
            ...
            applied = True
            reason = (
                f"{reason} Anti-drift aplicado: no hubo movimiento comercial verificado; "
                f"rotar temporalmente a {lane} para desafiar la estrategia de forma reversible."
            )
    elif triggered:
    ...
    return {
        ...
        "reason": reason,
        "anti_drift_applied": applied,
        "anti_drift_suggestion": suggestion or None,
        "anti_drift_guard_reason": guard_reason,
    }
...
def build_revenue_allocation(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "updated_at": utcnow(),
        "lane": lane,
        "base_lane": resolved["base_lane"],
        "reason": resolved["reason"],
        "success_metric": resolved["success_metric"],
        "target_weights": target_weights,
        "execution_attention_pct": 100 - explore,
        "exploration_attention_pct": explore,
        "anti_drift_applied": resolved["anti_drift_applied"],
        "anti_drift_suggestion": resolved["anti_drift_suggestion"],
        "anti_drift_guard_reason": resolved["anti_drift_guard_reason"],
...
def _run_with_revenue_allocation(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    report["revenue_allocator"] = {
        ...
        for key in (
            ...
            "success_metric",
            "reason",
            "execution_attention_pct",
            "exploration_attention_pct",
            "anti_drift_applied",
            "anti_drift_suggestion",
            "anti_drift_guard_reason",
...
print(
    {
        "revenue_allocator_runtime": {
            "version": VERSION,
            "status": "active",
            "canonical_truth": True,
            "guarded_anti_drift": True,
        }
    },
    flush=True,
)
