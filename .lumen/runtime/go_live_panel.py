...
def render_go_live(state: Dict[str, Any]) -> str:
    ...
    # Transport readiness and commercial release are deliberately separate truths.
    # A Closer HOLD or a go-live lane gate must never be presented as if email transport were down.
    readiness = state.get("external_market_readiness", {}) or {}
    outbound_engine = state.get("outbound_engine", {}) or {}
    transport = state.get("mail_transport_health", {}) or {}
    closer = state.get("closer_orchestrator", {}) or {}
    transport_ready = bool(
        (readiness.get("outbound_live") and readiness.get("mail_transport_ready"))
        or (outbound_engine.get("live") and transport.get("ok"))
    )
    transport_provider = readiness.get("mail_provider") or transport.get("provider") or "—"
    gate_enabled = bool(report.get("outbound_allowed"))
    closer_stage = str(closer.get("go_live_stage") or closer.get("status") or "—")
    ...
    <section class="gl-wrap">
      ...
      <div class="gl-grid">
        ...
        <div class="gl-box"><h3>Estado de control</h3>
          <p><b>Infraestructura outbound:</b> {'ACTIVA' if transport_ready else 'NO LISTA'} <small>· {_esc(transport_provider)}</small></p>
          <p><b>Gate GO-LIVE nuevos envíos:</b> {'HABILITADO' if gate_enabled else 'EN ESPERA'}</p>
          <p><b>Closer comercial:</b> {_esc(closer_stage)}</p>
          <p><b>Salud operativa:</b> {_esc(infra.get('operations_health_score'))}/100</p>
          <p><b>Modo:</b> {_esc(report.get('requested_mode'))} · autopromoción {'sí' if report.get('auto_promote') else 'no'}</p>
          <div class="gl-chips">{blocker_html}{warning_html}</div>
        </div>
      </div>
      <div class="gl-foot">Infraestructura outbound ≠ liberación del Closer: el transporte puede estar operativo mientras un carril comercial permanece en HOLD/espera por gobernanza. CANARIO: 1 envío/ciclo → LIMITADO: 2 → GOBERNADO: máximo constitucional. Contratos, pagos, órdenes y aceptación vinculante siguen requiriendo autoridad humana.</div>
    </section>
    """
