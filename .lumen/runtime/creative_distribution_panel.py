...
def snapshot(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "owned_live": [x for x in proof if x.get("status") == "published_owned_channel"],
...
def render_growth_ops_strip(state: Dict[str, Any]) -> str:
    ...
    <section class='growthops-strip'>
      <div><div class='go-eye'>CREATIVE FACTORY · DISTRIBUTION PROOF</div><h2>Crear excelente + demostrar que realmente salió</h2><p>LUMEN separa preparado, publicado, visto, click y lead. No cuenta una pieza externa como publicada sin comprobante.</p></div>
    ...
    for row in rows[:36]:
        ...
        <article class='creative-card'>
          <div class='preview {ratio}'>
            <div class='preview-eye'>{_e(row.get('eyebrow'))}</div>
            <h3>{_e(row.get('headline'))}</h3>
            <p>{_e(row.get('body'))}</p>
            <div class='proofline'>{_e(row.get('credibility_line'))}</div>
            <div class='cta'>{_e(row.get('cta'))}</div>
            <div class='brandmark'>LUMEN</div>
          </div>
          <div class='meta'>
            <div class='tag'>{'CHAMPION' if row.get('is_champion') else 'EXPERIMENTO'} · {_e(row.get('format_label'))}</div>
            <b>{_e(row.get('campaign_id'))} · {_e(row.get('angle'))}</b>
            <span>{_i(row.get('width'))}×{_i(row.get('height'))} · calidad {_f(row.get('quality_score')):.0f}/100</span>
            <p>{_e(direction.get('motif'))}</p>
            <code>{_e(row.get('tracking_path'))}</code>
          </div>
        </article>""")
    return "".join(out)
...
def _proof_rows(rows: List[Dict[str, Any]]) -> str:
    ...
    for row in rows[-120:][::-1]:
        out.append(f"""
        <div class='proof-row'>
          <div><div class='status {_e(row.get('status'))}'>{_e(labels.get(str(row.get('status') or ''), row.get('status')))}</div><b>{_e(row.get('campaign_id'))} · {_e(row.get('channel'))}</b><span>{_e(row.get('variant_id'))} · prueba: {_e(row.get('proof_level'))}</span></div>
          ...
          <div class='evidence'>{('<a href="'+_e(row.get('external_url'))+'" target="_blank" rel="noopener">Ver publicación externa ↗</a>') if row.get('external_url') else '<span>Sin URL externa verificada</span>'}<small>{_e(row.get('provider') or '')}</small></div>
        </div>""")
    return "".join(out)
