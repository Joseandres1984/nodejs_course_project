...
def render_creative_factory_page(state: Dict[str, Any]) -> str:
    s = snapshot(state)
    body = f"""<main class='wrap'><section class='top'><div><div class='brand'>LUMEN · CREATIVE FACTORY</div><h1>La fábrica de piezas comerciales</h1><p class='sub'>Cada campaña se convierte en piezas adaptadas por formato, con una sola promesa, evidencia prudente, CTA atribuible y dirección visual.</p></div><a class='btn' href='/command-center'>← Command Center</a></section><section class='stats'><div class='stat'><small>Piezas</small><b>{len(s['assets'])}</b></div><div class='stat'><small>Champions</small><b>{len(s['champions'])}</b></div><div class='stat'><small>Calidad media</small><b>{_f(s['factory'].get('quality_avg')):.0f}</b></div><div class='stat'><small>Render-ready</small><b>{_i(s['factory'].get('render_ready'))}</b></div></section><section class='grid'>{_creative_cards(s['assets'])}</section></main>"""
    return "<!doctype html><html lang='es'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><meta http-equiv='refresh' content='45'><title>LUMEN · Creative Factory</title><style>" + CREATIVE_CSS + "</style></head><body>" + body + "</body></html>"
...
def _proof_rows(rows: List[Dict[str, Any]]) -> str:
    ...
    for row in rows[-120:][::-1]:
        ...
        evidence_html = (
            f"<a href='{_e(row.get('external_url'))}' target='_blank' rel='noopener'>Ver publicación externa ↗</a>"
            if row.get("external_url") else "<span>Sin URL externa verificada</span>"
        )
        out.append(f"""
        <div class='proof-row'>
