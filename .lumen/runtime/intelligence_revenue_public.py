...
VERSION = "1.0-intelligence-public-intake"
...
def _page(result: str = "", selected: str = "") -> str:
    notice = ""
    if result == "received":
        notice = "<div class='notice'>Consulta recibida. El caso quedó registrado para verificación y diagnóstico. No se realizó ningún cargo.</div>"
...
<p class='lead'>Analizamos cotizaciones, proveedores, mercados de exportación y oportunidades públicas. Cada conclusión debe quedar respaldada por evidencia; lo que no está verificado se marca como dato faltante.</p>
<div class='proof'><span class='pill'>Multimoneda</span><span class='pill'>Sin compra automática</span><span class='pill'>Sin promesas inventadas</span><span class='pill'>Investigación global</span><span class='pill'>Precio final confirmado antes de contratar</span></div>
</section>
<section class='grid'>{''.join(cards)}</section>
<section class='formbox' id='consulta'>
<div class='eyebrow'>Diagnóstico inicial</div><h2>Contanos el problema comercial</h2>
<p class='small'>Sólo necesitamos un email y una descripción. Los datos adicionales ayudan a acelerar el análisis, pero son opcionales.</p>
...
<button type='submit'>Crear caso de análisis</button>
</form>
<p class='fine'>El envío no crea una contratación ni un cobro. Las referencias, verificaciones y oportunidades sólo se reportan cuando existe evidencia suficiente. Precios, contratos, pagos, compras y participaciones vinculantes requieren confirmación humana.</p>
</section>
...
def intelligence_status():
    ...
    return {
        ...
        "products": [
            {"id": x["id"], "name": x["name"], "status": x["status"], "promise": x["promise"]}
