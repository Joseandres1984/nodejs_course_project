...
LOW_VALUE_HOSTS = {"facebook.com", "instagram.com", "linkedin.com", "youtube.com", "reddit.com", "mercadolibre.com.ar", "mercadolibre.com", "amazon.com", "pinterest.com", "google.com", "bing.com", "yahoo.com"}
AFFILIATE_TERMS = ("afiliado", "afiliados", "affiliate", "partners", "partner", "referidos", "revendedor", "revendedores", "programa de afiliados", "programa de partners")
...
def _partner_id(domain: str) -> str:
    return "PST-" + hashlib.sha1(domain.lower().encode("utf-8")).hexdigest()[:14].upper()
...
def _consume_search(state: Dict[str, Any], cycle: Dict[str, int]) -> bool:
    ...
    if int(local.get("searches_remaining") or 0) <= 0 or int(global_budget.get("general_queries_remaining") or 0) <= 0:
        return False
...
def _ensure_store(stores: List[Dict[str, Any]], by_domain: Dict[str, Dict[str, Any]], domain: str, *, name: str = "", source: str = "", url: str = "", category: str = "", family: str = "") -> Optional[Dict[str, Any]]:
    ...
    if not store:
        store = {"id": _partner_id(domain), "domain": domain, "name": _clean(name, 180) or domain, "commercial_status": "prospect", "agreement_required": True, "commission_authorized": False, "commission_model": None, "commission_rate": None, "source_families": [], "source_urls": [], "categories": [], "catalog_products": [], "catalog_searches": 0, "catalog_last_checked_at": None, "affiliate_program_hint": False, "affiliate_evidence": [], "contact_hints": [], "created_at": utcnow(), "updated_at": utcnow(), "next_action": "Investigar catálogo y vía comercial antes de proponer partnership"}
        stores.append(store)
        by_domain[domain] = store
    if name and store.get("name") in {None, "", domain}:
        store["name"] = _clean(name, 180)
    for value in (family, source):
        if value and value not in store["source_families"]:
            store["source_families"].append(value)
...
def _add_product(store: Dict[str, Any], *, title: str, url: str, snippet: str = "", category: str = "", source: str = "search_index") -> bool:
    title, url = _clean(title, 260), _clean(url, 900)
    ...
    if not path or path in {"contacto", "contact", "nosotros", "about", "blog", "ayuda"}:
        return False
    key = _product_key(domain, url, title)
    products = store.setdefault("catalog_products", [])
    existing = next((x for x in products if x.get("key") == key or x.get("url") == url), None)
    row = {"key": key, "title": title, "url": url, "snippet": _clean(snippet, 700), "category": _clean(category, 160), "price_mentions": _price_mentions(f"{title} {snippet}"), "source": source, "observed_at": utcnow(), "eligible_for_publication": False, "publication_reason": "partner_agreement_required"}
    if existing:
...
def _sync_existing_evidence(state: Dict[str, Any]) -> int:
    ...
    for ev in state.get("retail_market_evidence", []) or []:
        ...
        if family == "mercadolibre":
            continue
        store = _ensure_store(stores, by_domain, domain, name=_clean(ev.get("source_title"), 180), source="retail_market_evidence", url=url, category=_clean(ev.get("product"), 160), family=family)
        if store and url:
            _add_product(store, title=_clean(ev.get("product") or ev.get("source_title"), 260), url=url, snippet=_clean(ev.get("source_snippet"), 700), category=_clean(ev.get("product"), 160), source="retail_market_evidence")
    for account in state.get("candidate_accounts", []) or []:
        if account.get("type") == "supplier":
            url = _clean(account.get("source_url"), 900)
            _ensure_store(stores, by_domain, _clean(account.get("domain"), 220) or _host(url), name=_clean(account.get("name_hint"), 180), source="supplier_account", url=url, category=_clean(account.get("category"), 160), family="b2b_supplier")
    for lead in state.get("research_leads", []) or []:
        if lead.get("type") == "supplier":
            url = _clean(lead.get("url"), 900)
            _ensure_store(stores, by_domain, _clean(lead.get("domain"), 220) or _host(url), name=_clean(lead.get("title"), 180), source="supplier_research", url=url, category=_clean(lead.get("category"), 160), family="supplier_research")
...
def _apply_agreements(state: Dict[str, Any]) -> int:
    ...
    for store in state.get("partner_stores", []) or []:
        agreement = by_domain.get(str(store.get("domain") or "").lower())
        active = False
        if agreement:
            status = str(agreement.get("status") or "").lower()
            active = status in {"active", "approved", "executed", "enrolled"} and bool(agreement.get("commission_pct") or agreement.get("commission_rate") or agreement.get("fixed_fee") or agreement.get("fixed_fee_usd") or agreement.get("affiliate_url_template") or agreement.get("affiliate_base_url"))
        ...
        if active:
            ...
            store["next_action"] = "Evaluar/enrolarse en el programa; aceptar términos requiere autorización humana"
        else:
...
def _search_store(state: Dict[str, Any], store: Dict[str, Any], cycle: Dict[str, int]) -> Dict[str, int]:
    ...
    query = f"site:{domain} {category_clause}(comprar OR producto OR tienda OR catálogo OR catalogo OR stock)"
    try:
        ...
        return {"products": 0, "affiliate": 0, "contact": 0}
    products = affiliate = contact = 0
    for item in results:
        url, title, snippet = _clean(item.get("url"), 900), _clean(item.get("title"), 260), _clean(item.get("snippet"), 700)
        ...
        if any(term in text for term in AFFILIATE_TERMS):
            ...
            if url and not any(x.get("url") == url for x in store.setdefault("affiliate_evidence", [])):
                store["affiliate_evidence"].append({"url": url, "title": title, "snippet": snippet, "observed_at": utcnow()})
                store["affiliate_evidence"] = store["affiliate_evidence"][-10:]
                affiliate += 1
        if any(term in text for term in CONTACT_TERMS) and url and url not in store.setdefault("contact_hints", []):
    ...
    for store in state.get("partner_stores", []) or []:
        ...
        for product in store.get("catalog_products", []) or []:
            ...
            if not row:
                ...
                row = {"id": f"PROF-{len(offers)+1:05d}", "offer_key": offer_key, "token": token, "partner_id": store.get("id"), "partner_domain": store.get("domain"), "partner_name": store.get("name"), "product_key": product.get("key"), "title": product.get("title"), "category": product.get("category"), "source_url": product.get("url"), "price_mentions": product.get("price_mentions", []), "commission_model": store.get("commission_model"), "commission_rate": store.get("commission_rate"), "agreement_id": store.get("agreement_id"), "status": "active", "lumen_referral_path": f"/go/{token}", "created_at": utcnow(), "updated_at": utcnow()}
                offers.append(row)
                known[offer_key] = row
                created += 1
            else:
                row.update({"title": product.get("title"), "source_url": product.get("url"), "price_mentions": product.get("price_mentions", []), "status": "active", "commission_model": store.get("commission_model"), "commission_rate": store.get("commission_rate"), "agreement_id": store.get("agreement_id"), "updated_at": utcnow()})
    state["partner_referral_offers"] = offers[-MAX_ACTIVE_OFFERS:]
    return created


def partner_network_tick(state: Dict[str, Any]) -> Dict[str, Any]:
    """Map stores/catalogs and prepare monetized referrals only after explicit partner authorization."""
    created_stores = _sync_existing_evidence(state)
    activated = _apply_agreements(state)
    cycle = {"used": 0}
    searched = products_added = affiliate_hints = contacts = 0
    stores = list(state.get("partner_stores", []) or [])
    prospects = sorted(stores, key=lambda x: (0 if x.get("commission_authorized") else 1, int(x.get("catalog_searches") or 0), -len(x.get("source_urls", []) or []), str(x.get("domain") or "")))
    for store in prospects[:MAX_STORES_PER_CYCLE]:
        if cycle["used"] >= MAX_SEARCHES_PER_CYCLE or int(_budget(state).get("searches_remaining") or 0) <= 0 or int(agent_fleet._budget(state).get("general_queries_remaining") or 0) <= 0:
            break
    ...
    report = {"version": "1.0", "updated_at": utcnow(), "status": "healthy", "stores_total": len(state.get("partner_stores", []) or []), "stores_created": created_stores, "stores_searched": searched, "products_added": products_added, "affiliate_hints_added": affiliate_hints, "contact_hints_added": contacts, "active_partners": sum(1 for x in state.get("partner_stores", []) or [] if x.get("commercial_status") == "active_partner"), "agreement_required": sum(1 for x in state.get("partner_stores", []) or [] if x.get("agreement_required")), "partners_activated": activated, "referral_offers_active": sum(1 for x in state.get("partner_referral_offers", []) or [] if x.get("status") == "active"), "referral_offers_created": offers_created, "searches_used": cycle["used"], "partner_search_budget_remaining": int(_budget(state).get("searches_remaining") or 0), "global_general_search_remaining": int(agent_fleet._budget(state).get("general_queries_remaining") or 0), "commercial_rule": "buyer_pays_store_store_fulfills_lumen_receives_only_agreed_commission", "publication_gate": "active_referral_requires_explicit_partner_agreement"}
    state["partner_network"] = report
    state.setdefault("activity", []).insert(0, {"ts": utcnow(), "msg": f"Partner Network: {report['stores_total']} tiendas mapeadas, {report['active_partners']} partners activos, {report['referral_offers_active']} ofertas atribuibles; {cycle['used']} búsquedas de catálogo este ciclo."})
