...
def _risk_profile(deal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    severe = {"payment_instructions_unverified", "buyer_identity_unverified", "supplier_identity_unverified"}
    penalty = min(0.55, len(flags) * 0.055 + sum(0.07 for x in flags if x in severe))
    return {
        "flags": flags,
        "risk_penalty_pct": round(penalty * 100.0, 1),
        "risk_factor": round(max(0.35, 1.0 - penalty), 3),
    }
...
def _deal_financials(state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for deal in state.get("deals", []):
        ...
        # Money score rewards risk-adjusted profit, probability, margin and capital efficiency.
        ...
        rows.append({
            ...
            "risk_adjusted_expected_profit_usd": round(risk_adjusted_profit, 2),
            "modeled_supplier_cost_if_won_usd": round(modeled_supplier_exposure, 2),
            "capital_efficiency": round(capital_efficiency, 3),
            "risk_flags": risk["flags"],
            "risk_penalty_pct": risk["risk_penalty_pct"],
            "money_score": round(max(0.0, min(100.0, score)), 2),
        })
...
def _concentration(rows: List[Dict[str, Any]], field: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "risk": "high" if top_share >= 55 else "medium" if top_share >= 35 else "low",
    }
...
def _transactions(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for txn in state.get("transactions", []):
        ...
        if status in {"closed_simulated", "simulated"}:
            continue
        if status not in REAL_TRANSACTION_STATUSES:
            continue
        ...
        if status in REALIZED_STATUSES:
            ...
            unsettled.append({
                ...
                "status": status,
                "gross_receivable_usd": round(sale, 2),
                "company_profit_usd": round(profit, 2),
            })
    return {
        "committed_profit_usd": round(committed_profit, 2),
        "realized_profit_usd": round(realized_profit, 2),
        "committed_supplier_cost_usd": round(committed_supplier_cost, 2),
        "gross_unsettled_receivable_usd": round(gross_receivable, 2),
...
def cfo_tick(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    warnings: List[str] = []
    if buyer_conc["risk"] == "high":
        warnings.append("buyer_concentration_high")
    if supplier_conc["risk"] == "high":
        warnings.append("supplier_concentration_high")
    if category_conc["risk"] == "high":
        warnings.append("category_concentration_high")
    if _f(tx.get("gross_unsettled_receivable_usd")) > 0:
        warnings.append("collection_exposure_present")
    if any("payment_instructions_unverified" in x.get("risk_flags", []) for x in rows[:5]):
        warnings.append("payment_controls_incomplete_on_top_deals")

    snapshot = {
        "pipeline_sale_price_usd": round(pipeline_sale, 2),
        "pipeline_company_profit_usd": round(pipeline_profit, 2),
        "probability_weighted_profit_usd": round(expected_profit, 2),
        "risk_adjusted_expected_profit_usd": round(risk_adjusted, 2),
        "modeled_supplier_cost_if_pipeline_won_usd": round(modeled_supplier_exposure, 2),
        "pending_human_approval_profit_usd": round(pending_approval_profit, 2),
        **tx,
    ...
    report = {
        ...
        "concentration": {
            "buyers": buyer_conc,
            "suppliers": supplier_conc,
            "categories": category_conc,
        },
        "warnings": warnings,
        "capital_doctrine": [
            "priorizar beneficio esperado ajustado por riesgo, no facturación bruta",
    ...
    if top:
        record_decision(
            ...
            engine="LUMEN CFO",
            object_type="deal",
            object_id=str(top.get("deal_id") or ""),
            decision="capital_priority_ranked",
            reason=(
                f"Mayor prioridad financiera actual: beneficio esperado ajustado por riesgo USD "
                f"{top.get('risk_adjusted_expected_profit_usd', 0):,.2f}; money score {top.get('money_score', 0):.1f}."
            ),
            action="score_opportunity",
            confidence=max(0.5, min(0.98, _f(top.get("close_probability"), 0.5))),
            evidence_refs=[],
        )
    return report
