...
def _profile_shell(account: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "name": account.get("company_name") or account.get("name_hint") or account.get("site_title") or account.get("domain"),
        "type": account.get("type"),
        "domain": account.get("domain"),
        "observed_events": 0,
        "outbound_sent": 0,
        "responses": 0,
        "negotiation_requests": 0,
        "negotiation_responses": 0,
        "offer_revisions": 0,
        "price_improvements": 0,
        "price_improvement_total_pct": 0.0,
        "lead_time_improvements": 0,
        "payment_term_changes": 0,
        "buyer_price_objections": 0,
        "buyer_delivery_questions": 0,
        "buyer_interest_signals": 0,
...
def _build_profiles(state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for incoming in state.get("inbox", []) or []:
        ...
        if kind == "price_objection":
            p["buyer_price_objections"] += 1
        elif kind == "delivery_question":
            p["buyer_delivery_questions"] += 1
        elif kind == "buyer_interest":
            p["buyer_interest_signals"] += 1
        source_id = str(incoming.get("revops_source_message_id") or "")
        if source_id:
            source = next((x for x in state.get("outbox", []) if str(x.get("id") or "") == source_id), {})
            if source.get("kind") == "supplier_negotiation":
                p["negotiation_responses"] += 1

    grouped_offers: Dict[tuple[str, str], List[Dict[str, Any]]] = defaultdict(list)
    for offer in state.get("offers", []) or []:
        ...
        if account_id and deal_id and account_id in profiles:
            grouped_offers[(account_id, deal_id)].append(offer)

    for (account_id, _deal_id), offers in grouped_offers.items():
        offers.sort(key=_offer_time)
        p = profiles[account_id]
        for prev, cur in zip(offers, offers[1:]):
            p["offer_revisions"] += 1
            if str(prev.get("currency") or "") == str(cur.get("currency") or ""):
                old_amount = _f(prev.get("amount"))
                new_amount = _f(cur.get("amount"))
                if old_amount > 0 and 0 < new_amount < old_amount:
                    improvement = (old_amount - new_amount) / old_amount * 100.0
                    p["price_improvements"] += 1
                    p["price_improvement_total_pct"] += improvement
            old_lead = _f(prev.get("lead_days"))
            new_lead = _f(cur.get("lead_days"))
            if old_lead > 0 and 0 < new_lead < old_lead:
                p["lead_time_improvements"] += 1
            old_terms = _norm(prev.get("payment_terms"))
            new_terms = _norm(cur.get("payment_terms"))
    ...
    for account_id, p in profiles.items():
        outbound = _i(p.get("outbound_sent"))
        responses = _i(p.get("responses"))
        negotiation_requests = _i(p.get("negotiation_requests"))
        negotiation_responses = _i(p.get("negotiation_responses"))
        price_improvements = _i(p.get("price_improvements"))
        revisions = _i(p.get("offer_revisions"))
        ...
        p["negotiation_response_rate"] = round(negotiation_responses / max(1, negotiation_requests), 3) if negotiation_requests else None
        ...
        if p.get("type") == "supplier":
            signals = {
                "price_flexibility": price_improvements,
                "lead_time_flexibility": _i(p.get("lead_time_improvements")),
                "payment_term_flexibility": _i(p.get("payment_term_changes")),
            }
            strongest = max(signals, key=signals.get) if max(signals.values(), default=0) > 0 else "unknown"
            p["observed_supplier_flexibility"] = strongest
        else:
            priorities = {
                "price": _i(p.get("buyer_price_objections")),
                "delivery": _i(p.get("buyer_delivery_questions")),
                "interest": _i(p.get("buyer_interest_signals")),
            }
        ...
        account["negotiation_behavior"] = {
            ...
            "personalization_ready": p["personalization_ready"],
            "response_rate": p.get("response_rate"),
            "observed_supplier_flexibility": p.get("observed_supplier_flexibility"),
            "observed_buyer_priority": p.get("observed_buyer_priority"),
            "updated_at": p["updated_at"],
        }
        out.append(p)

    out.sort(key=lambda x: (_f(x.get("confidence")), _i(x.get("observed_events"))), reverse=True)
...
def _negotiation_rounds(state: Dict[str, Any], deal_id: str, supplier_id: str) -> int:
    return sum(
        ...
        if x.get("kind") == "supplier_negotiation"
        and str(x.get("deal_id") or "") == deal_id
        and str(x.get("counterparty_account_id") or "") == supplier_id
        and x.get("status") in {"ready", "sent"}
    )


def _profile_index(profiles: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    return {str(x.get("account_id")): x for x in profiles if x.get("account_id")}


def _plan_for_deal(state: Dict[str, Any], deal: Dict[str, Any], profiles: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    deal_id = str(deal.get("id") or "")
    buyer_id = str(deal.get("buyer_account_id") or "")
    supplier_id = str(deal.get("supplier_account_id") or "")
    buyer = profiles.get(buyer_id, {})
    supplier = profiles.get(supplier_id, {})
    ...
    rounds = _negotiation_rounds(state, deal_id, supplier_id)
    safe_score = _f(safeguards.get("safe_close_score"), 100.0)
    buyer_priority = str(buyer.get("observed_buyer_priority") or "unknown")
    supplier_flex = str(supplier.get("observed_supplier_flexibility") or "unknown")
    supplier_conf = _f(supplier.get("confidence"))

    if deal.get("incident_hold") or deal.get("legal_review_required") or safe_score < 55:
        strategy = "HOLD"
        objective = "repair_risk"
        reason = "Deal Safeguards domina cualquier negociación económica."
    elif rounds >= MAX_NEGOTIATION_ROUNDS:
        strategy = "WALK_AWAY_FROM_FURTHER_PRESSURE"
        objective = "preserve_relationship"
        reason = "Se alcanzó el límite de rondas autónomas; no erosionar relación ni crear presión repetitiva."
    elif action == "IMPROVE_MARGIN":
        if supplier_conf >= 0.35 and supplier_flex == "price_flexibility":
            strategy = "TARGET_PRICE"
            objective = "supplier_price"
            reason = "Hay evidencia observada de flexibilidad de precio y el deal necesita mejorar margen."
        elif supplier_conf >= 0.35 and supplier_flex == "payment_term_flexibility":
            strategy = "TARGET_TERMS"
            objective = "payment_terms"
            reason = "La contraparte mostró más flexibilidad en términos que en precio; mejorar economía sin asumir descuento."
        elif supplier_conf >= 0.35 and supplier_flex == "lead_time_flexibility":
            strategy = "TARGET_DELIVERY"
            objective = "lead_time"
            reason = "La flexibilidad histórica observada está en plazo; usarla para aumentar valor sin sacrificar margen comprador."
        else:
            strategy = "NEUTRAL_BEST_TERMS"
            objective = "best_real_condition"
            reason = "Historia insuficiente: pedir mejor condición real sin inventar palancas."
    elif action == "REPAIR_TERMS":
        strategy = "TARGET_TERMS"
        objective = "risk_alignment"
        reason = "La prioridad económica es reparar términos/Safe Close antes de acelerar."
    elif action == "ACCELERATE":
        if buyer_priority == "delivery" and supplier_flex == "lead_time_flexibility" and supplier_conf >= 0.35:
            strategy = "TARGET_DELIVERY"
            objective = "lead_time"
            reason = "Comprador muestra interés en entrega y proveedor evidencia flexibilidad de plazo."
        else:
            strategy = "PRESERVE_MARGIN"
            objective = "close_without_unnecessary_concession"
            reason = "El deal ya es económicamente atractivo; evitar descuentos no solicitados."
    else:
        strategy = "NEUTRAL_BEST_TERMS"
        objective = "best_real_condition"
        reason = "Mejorar conversión con una solicitud prudente y no vinculante."

    expected_leverage = "unknown"
    if supplier.get("personalization_ready"):
        if strategy == "TARGET_PRICE" and supplier.get("avg_observed_price_improvement_pct") is not None:
            expected_leverage = f"observed_price_improvement_avg:{supplier.get('avg_observed_price_improvement_pct')}%"
        elif strategy == "TARGET_DELIVERY":
            expected_leverage = "observed_lead_time_flexibility"
        elif strategy == "TARGET_TERMS":
            expected_leverage = "observed_payment_term_flexibility"
    ...
    return {
        ...
        "objective": objective,
        "reason": reason,
        "autonomous": strategy not in {"HOLD", "WALK_AWAY_FROM_FURTHER_PRESSURE"},
        "negotiation_rounds": rounds,
        "max_autonomous_rounds": MAX_NEGOTIATION_ROUNDS,
        "buyer_observed_priority": buyer_priority,
        "buyer_confidence": buyer.get("confidence"),
        "supplier_observed_flexibility": supplier_flex,
        "supplier_confidence": supplier.get("confidence"),
        "expected_leverage": expected_leverage,
        "defended_target_margin_pct": capital.get("defended_target_margin_pct") or deal.get("defended_target_margin_pct"),
        "current_margin_pct": capital.get("margin_pct") or (deal.get("economics", {}) or {}).get("company_share_pct") or deal.get("company_share_pct"),
        "safe_close_score": safe_score,
        "updated_at": utcnow(),
        "governance": {
            "nonbinding_only": True,
            "no_fake_competition": True,
            "no_fake_urgency": True,
            "no_unverified_concession_claims": True,
            "binding_acceptance_requires_human": True,
        },
    }
...
def negotiation_intelligence_tick(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile_map = _profile_index(profiles)
    ...
    for deal in state.get("deals", []) or []:
        ...
        if plan.get("deal_id"):
            plans.append(plan)
            deal["negotiation_strategy"] = plan.get("strategy")
            deal["negotiation_objective"] = plan.get("objective")
            deal["negotiation_reviewed_at"] = plan.get("updated_at")
    ...
    plans.sort(key=lambda x: (priority.get(str(x.get("strategy")), 0), _f(x.get("supplier_confidence"))), reverse=True)
    plans = plans[:MAX_PLANS]
    state["counterparty_behavior_profiles"] = profiles
    state["negotiation_plan_index"] = {str(x.get("deal_id")): x for x in plans if x.get("deal_id")}

    primary = plans[0] if plans else None
    report = {
        "updated_at": utcnow(),
        "mode": "negotiation_intelligence_2",
        "profiles": profiles,
        "plans": plans,
        "profiles_total": len(profiles),
        "personalized_profiles": sum(1 for x in profiles if x.get("personalization_ready")),
        ...
        "governance": {
            ...
            "authority_rule": "Negotiation may request nonbinding improvements only; accepting terms, discounts, orders, contracts or payments remains human-gated.",
    ...
    if primary:
        record_decision(
            state,
            engine="Negotiation Intelligence 2.0",
            object_type="deal",
            object_id=str(primary.get("deal_id")),
            decision=f"strategy:{str(primary.get('strategy') or '').lower()}",
            reason=str(primary.get("reason") or "Behavioral strategy selected from observed evidence."),
            action="negotiate_nonbinding" if primary.get("autonomous") else "score_opportunity",
            confidence=max(0.45, min(0.96, _f(primary.get("supplier_confidence"), 0.5))),
            evidence_refs=[],
